...
$filter_type = isset($_GET['filter_type']) ? $_GET['filter_type'] : 'entry_date'; // entry_date or update_date
...
// Get last update date from update_history
function getLastUpdateDate($seller) {
    if (!empty($seller['update_history'])) {
        $history = json_decode($seller['update_history'], true);
        if (is_array($history) && !empty($history)) {
            $lastEntry = $history[0];
            if (isset($lastEntry['timestamp'])) {
                return date('d M Y', strtotime($lastEntry['timestamp']));
            }
        }
    }
    return '-';
...
<head>
    ...
    <title>CNP Sellers - Work Station</title>
                                    ...
                                    <div class="col-12 col-md-4">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" 
                                                   name="search" 
                                                   placeholder="Search by name, phone, queries, seller type..." 
                                                    ...
                                                    <td colspan="8" class="text-center py-5">
                                                        <div class="py-4">
                                                            ...
                                                            <p class="text-muted mb-3">
                                                                ...
                                                                <?php else: ?>
                                                                    No call not picked records available
                                                                <?php endif; ?>
                                                            </p>
                                                            <?php if (!empty($search) || !empty($from_date) || !empty($to_date)): ?>
                                                                <a href="cnp_sellers.php" class="btn btn-outline-secondary">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="px-3">
                                                            <?php if (!empty($seller['entry_date'])): ?>
                                                                <span class="badge bg-primary bg-opacity-10 text-primary entry-date-badge">
                                                        ...
                                                        <td class="px-3">
                                                            <?php 
                                                            $lastUpdate = getLastUpdateDate($seller);
                                                            if ($lastUpdate != '-'): ?>
                                                                <span class="badge bg-info bg-opacity-10 text-info entry-date-badge">
                                                        ...
                                                        <td class="px-3">
                                                            <div class="text-truncate" style="max-width: 200px;" title="<?= htmlspecialchars($seller['latest_update'] ?? 'N/A') ?>">
