...
try {
    ...
    if ($action === 'get_data') {
        ...
        $duration_filter = $_POST['duration_filter'] ?? '';
        
        $offset = ($page - 1) * $per_page;
        
        // Build query - Only Plan Upgraded responses
        $where = "WHERE user_uid = ? AND customer_response = 'Plan Upgraded'";
        $params = [$user_uid];
        
        // Add plan filter
        if (!empty($plan_filter)) {
            if ($plan_filter === 'other') {
                $where .= " AND plans_interested NOT IN ('Welcome', 'Starter', 'Professional', 'Intermediate')";
            } else {
        ...
        // Add duration filter (search in remembering_notes)
        if (!empty($duration_filter)) {
            $where .= " AND remembering_notes LIKE ?";
            $params[] = "%Upgraded Duration: $duration_filter%";
        }
        ...
        // Get stats for Plan Upgraded only
        $stats_sql = "SELECT 
            COUNT(*) as total,
            ...
            WHERE user_uid = ? AND customer_response = 'Plan Upgraded'";
    ...
    if ($action === 'delete') {
        ...
        $stmt = $pdo->prepare("DELETE FROM sales_person_sellers 
                               WHERE id = ? AND user_uid = ? AND customer_response = 'Plan Upgraded'");
        $result = $stmt->execute([$id, $user_uid]);
        
        if ($result && $stmt->rowCount() > 0) {
            echo json_encode(['status' => 'success', 'message' => 'Upgraded seller record deleted successfully']);
        } else {
    ...
    error_log("Database Error in upgraded_sellers_ajax: " . $e->getMessage());
    echo json_encode(['status' => 'error', 'message' => 'Database error occurred']);
} catch (Exception $e) {
    error_log("General Error in upgraded_sellers_ajax: " . $e->getMessage());
