...
try {
    ...
    if ($action === 'get_data') {
        ...
        // Build query - Only Switch Off responses
        $where = "WHERE user_uid = ? AND customer_response = 'Switch Off'";
        $params = [$user_uid];
    ...
    if ($action === 'delete') {
        ...
        $stmt = $pdo->prepare("DELETE FROM sales_person_sellers 
                               WHERE id = ? AND user_uid = ? AND customer_response = 'Switch Off'");
    ...
    error_log("Database Error in switchoff_ajax: " . $e->getMessage());
    echo json_encode(['status' => 'error', 'message' => 'Database error occurred']);
} catch (Exception $e) {
    error_log("General Error in switchoff_ajax: " . $e->getMessage());
