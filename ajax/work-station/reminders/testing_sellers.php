...
try {
    ...
    if ($action === 'get_data') {
        ...
        // Build query - Only Testing responses
        $where = "WHERE user_uid = ? AND customer_response = 'Testing'";
        $params = [$user_uid];
        ...
        if (!empty($search)) {
            $where .= " AND (work_details_update LIKE ? OR phone_number LIKE ? OR customer_queries LIKE ? OR remembering_notes LIKE ?)";
            $params[] = "%$search%";
            $params[] = "%$search%";
            $params[] = "%$search%";
            $params[] = "%$search%";
        ...
        $stats_sql = "SELECT 
            ...
            WHERE user_uid = ? AND customer_response = 'Testing'";
    ...
    if ($action === 'delete') {
        ...
        $stmt = $pdo->prepare("DELETE FROM sales_person_sellers 
                               WHERE id = ? AND user_uid = ? AND customer_response = 'Testing'");
        $result = $stmt->execute([$id, $user_uid]);
        
        if ($result && $stmt->rowCount() > 0) {
            echo json_encode(['status' => 'success', 'message' => 'Test record deleted successfully']);
        } else {
    ...
    error_log("Database Error in testing_ajax: " . $e->getMessage());
    echo json_encode(['status' => 'error', 'message' => 'Database error occurred']);
} catch (Exception $e) {
    error_log("General Error in testing_ajax: " . $e->getMessage());
