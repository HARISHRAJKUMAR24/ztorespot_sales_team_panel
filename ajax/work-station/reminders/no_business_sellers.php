...
try {
    ...
    if ($action === 'get_data') {
        ...
        // Build query - Only No Business responses
        $where = "WHERE user_uid = ? AND customer_response = 'No Business'";
        $params = [$user_uid];
        
        // Add business status filter (search in remembering_notes and customer_queries)
        if (!empty($status_filter)) {
            $status_keywords = [
                'closed' => '%closed% OR %shut% OR %not operating%',
                'not_started' => '%not started% OR %yet to start% OR %planning%',
                'seasonal' => '%seasonal% OR %season% OR %temporary%',
                'relocated' => '%relocate% OR %moved% OR %shift%',
                'other' => '%other% OR %reason% OR %because%'
            ];
            
            if (isset($status_keywords[$status_filter])) {
                $where .= " AND (remembering_notes LIKE ? OR customer_queries LIKE ?)";
                $params[] = $status_keywords[$status_filter];
                $params[] = $status_keywords[$status_filter];
            }
        }
        ...
        // Get stats for No Business only
        $stats_sql = "SELECT 
            COUNT(*) as total,
            ...
            WHERE user_uid = ? AND customer_response = 'No Business'";
    ...
    if ($action === 'delete') {
        ...
        $stmt = $pdo->prepare("DELETE FROM sales_person_sellers 
                               WHERE id = ? AND user_uid = ? AND customer_response = 'No Business'");
        $result = $stmt->execute([$id, $user_uid]);
        
        if ($result && $stmt->rowCount() > 0) {
            echo json_encode(['status' => 'success', 'message' => 'No business record deleted successfully']);
        } else {
    ...
    error_log("Database Error in no_business_ajax: " . $e->getMessage());
    echo json_encode(['status' => 'error', 'message' => 'Database error occurred']);
} catch (Exception $e) {
    error_log("General Error in no_business_ajax: " . $e->getMessage());
