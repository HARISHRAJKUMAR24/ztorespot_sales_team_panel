$(document).ready(function() {
    ...
    $('.view-seller').on('click', function() {
        ...
        $('#editFromModal').attr('href', BASE_URL + 'work-station/sheets_edit_seller.php?id=' + sellerId);
        
        // Fetch seller details
        $.ajax({
            url: BASE_URL + 'ajax/work-station/sales-person-sellers/get-seller-details.php',
            type: 'POST',
            data: { id: sellerId },
            dataType: 'json',
    ...
    function displaySellerDetails(seller) {
        ...
        // Determine customer response badge class
        const response = seller.customer_response || 'N/A';
        let responseClass = 'secondary';
        if (response === 'Not interested') responseClass = 'danger';
        else if (response === 'Later' || response === 'Call Back AT') responseClass = 'warning';
        else if (response === 'Whatsapp Details sent') responseClass = 'info';
        else if (response === 'Plan Upgraded') responseClass = 'success';
        else if (response === 'CNP') responseClass = 'dark';
        ...
        const html = `
            <div class="container-fluid px-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div><span class="badge bg-${responseClass}">${escapeHtml(seller.customer_response || 'N/A')}</span></div>
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="text-muted small mb-1">Products Uploaded</label>
                                <div>${seller.products_uploaded || 0}</div>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="text-muted small mb-1">Seller ID</label>
                                <div>${escapeHtml(seller.seller_id || 'Not assigned')}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 bg-light mt-3">
                    <div class="card-body">
                        <h6 class="card-title text-success mb-3">
                            <i class="bi bi-cloud-upload me-2"></i>Import Information
                        </h6>
                        <div class="row">
                            <div class="col-12">
                                <label class="text-muted small mb-1">Import Batch</label>
                                <div><span class="badge bg-secondary">${escapeHtml(seller.import_batch)}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
