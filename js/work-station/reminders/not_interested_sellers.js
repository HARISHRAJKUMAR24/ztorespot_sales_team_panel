$(document).ready(function () {
    ...
    function loadData() {
        ...
        $.ajax({
           url: BASE_URL + 'ajax/work-station/reminders/not_interested_sellers.php',
            type: 'POST',
            data: {
                action: 'get_data',
                page: currentPage,
    ...
    function renderTable(rows) {
        ...
        rows.forEach(function (row) {
            ...
            // Get reason from remembering_notes or customer_queries
            let reason = row.remembering_notes || row.customer_queries || 'No reason provided';
            ...
            html += `<td><span class="badge bg-warning bg-opacity-25 text-warning">${escapeHtml(row.source_type || 'N/A')}</span></td>`;
            html += `<td><small class="text-muted"><i class="bi bi-chat-quote me-1"></i>${escapeHtml(reason)}</small></td>`;
    ...
    function renderPagination(total, page, perPage) {
        ...
        if (totalPages > 1) {
            html += `<li class="page-item ${page === 1 ? 'disabled' : ''}">
                <a class="page-link bg-warning text-dark border-warning" href="#" data-page="${page - 1}">&laquo;</a>
            </li>`;
            ...
            for (let i = startPage; i <= endPage; i++) {
                html += `<li class="page-item ${i === page ? 'active' : ''}">
                    <a class="page-link ${i === page ? 'bg-warning text-dark border-warning' : 'text-warning'}" 
                       href="#" data-page="${i}">${i}</a>
                </li>`;
            }
            
            html += `<li class="page-item ${page === totalPages ? 'disabled' : ''}">
                <a class="page-link bg-warning text-dark border-warning" href="#" data-page="${page + 1}">&raquo;</a>
            </li>`;
        }
    ...
    function showSellerDetails(seller) {
        ...
        const fields = [
            ...
            ['Reason / Notes', seller.remembering_notes],
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        if (confirm('Are you sure you want to delete this not interested record?')) {
            $.ajax({
                url: BASE_URL + 'ajax/work-station/reminders/not_interested_sellers.php',
                type: 'POST',
