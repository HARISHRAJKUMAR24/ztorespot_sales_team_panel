$(document).ready(function () {
    ...
    $('#applyFilters').on('click', function () {
        ...
        durationFilter = $('#durationFilter').val();
        currentPage = 1;
        loadData();
    });
    ...
    function loadData() {
        ...
        $.ajax({
             url: BASE_URL + 'ajax/work-station/reminders/upgrade_sellers.php',
            type: 'POST',
            data: {
                action: 'get_data',
                page: currentPage,
    ...
    function renderTable(rows) {
        ...
        rows.forEach(function (row) {
            ...
            // Get plan from plans_interested
            let upgradedPlan = row.plans_interested || 'N/A';

            // Extract duration from remembering_notes
            let duration = 'N/A';
            if (row.remembering_notes) {
                const durationMatch = row.remembering_notes.match(/Upgraded Duration: ([^.]+)/);
                if (durationMatch && durationMatch[1]) {
                    duration = durationMatch[1].trim();
                }
            }

            // Get products uploaded
            let products = row.products_uploaded !== undefined && row.products_uploaded !== null ? row.products_uploaded : 0;
            ...
            html += `<td><span class="badge bg-success">${escapeHtml(upgradedPlan)}</span></td>`;
            html += `<td><span class="badge bg-info">${escapeHtml(duration)}</span></td>`;
            html += `<td><span class="badge products-badge"><i class="bi bi-files me-1"></i>${products}</span></td>`;
    ...
    function showSellerDetails(seller) {
        ...
        // Extract duration from remembering_notes
        let duration = 'N/A';
        if (seller.remembering_notes) {
            const durationMatch = seller.remembering_notes.match(/Upgraded Duration: ([^.]+)/);
            if (durationMatch && durationMatch[1]) {
                duration = durationMatch[1].trim();
            }
        }
        ...
        const fields = [
            ...
            ['Upgraded Plan', seller.plans_interested],
            ['Duration', duration],
        ...
        fields.forEach(function (field) {
            if (field[1] && field[1] !== '' && field[1] !== null && field[1] !== '-') {
                ...
                if (field[0] === 'Products Uploaded') {
                    value = `<span class="badge products-badge fs-6">${value}</span>`;
                }
                html += `
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        if (confirm('Are you sure you want to delete this upgraded seller record?')) {
            $.ajax({
                url: BASE_URL + 'ajax/work-station/reminders/upgrade_sellers.php',
                type: 'POST',
