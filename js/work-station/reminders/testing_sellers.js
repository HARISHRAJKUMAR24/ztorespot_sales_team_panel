$(document).ready(function () {
    ...
    function loadData() {
        ...
        $.ajax({
            url: BASE_URL + 'ajax/work-station/reminders/testing_sellers.php',
            type: 'POST',
            data: {
                action: 'get_data',
                page: currentPage,
    ...
    function renderTable(rows) {
        ...
        rows.forEach(function (row) {
            ...
            // Get test notes from customer_queries or remembering_notes
            let testNotes = row.customer_queries || row.remembering_notes || 'No notes';
            if (testNotes.length > 30) testNotes = testNotes.substring(0, 30) + '...';
            ...
            html += `<td><span class="badge bg-purple bg-opacity-25 text-purple">${escapeHtml(row.source_type || 'N/A')}</span></td>`;
            html += `<td><small class="text-muted">${escapeHtml(testNotes)}</small></td>`;
    ...
    function renderPagination(total, page, perPage) {
        ...
        if (totalPages > 1) {
            html += `<li class="page-item ${page === 1 ? 'disabled' : ''}">
                <a class="page-link bg-purple text-white border-purple" href="#" data-page="${page - 1}">&laquo;</a>
            </li>`;
            ...
            for (let i = startPage; i <= endPage; i++) {
                html += `<li class="page-item ${i === page ? 'active' : ''}">
                    <a class="page-link ${i === page ? 'bg-purple text-white border-purple' : 'text-purple'}" 
                       href="#" data-page="${i}">${i}</a>
                </li>`;
            }

            html += `<li class="page-item ${page === totalPages ? 'disabled' : ''}">
                <a class="page-link bg-purple text-white border-purple" href="#" data-page="${page + 1}">&raquo;</a>
            </li>`;
        }
    ...
    function showSellerDetails(seller) {
        ...
        const fields = [
            ...
            ['Test Notes / Queries', seller.customer_queries],
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        if (confirm('Are you sure you want to delete this test record?')) {
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        showToast('success', 'Success', 'Test record deleted successfully');
                        loadData();
                    } else {
                        showToast('danger', 'Error', response.message);
                    }
